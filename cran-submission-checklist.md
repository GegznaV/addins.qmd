...
`R CMD check` NOTE entries are reviewed and either fixed or
...
`devtools::document(roclets = c("rd", "collate", "namespace"))` has been
run.
...
Workflow files under `.github/workflows/` use current action majors
(`actions/checkout@v4`, `r-lib/actions/*@v3`).

Any artifact actions use `actions/upload-artifact@v4` and
