...
- `qmd_code_block()` - verbatim code block. **Note:** in Markdown Visual
  Editor mode does not work.

- `qmd_code_block_r()` - R code block. **Note!** In Markdown Visual
  Editor (MVE) mode works differently: in MVE mode, RStudio IDE command
  ...
  last line of the document in MVE mode, the selected text is
